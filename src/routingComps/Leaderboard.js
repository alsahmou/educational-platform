...
export default class LeaderboardComp extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("http://" + this.state.ip + ":5000/projects/getGraded")
      .then((response) => {
      ...
      for (i = 0; i < rawProjects.length; i++) {
        if (rawProjects[i].username in usersPoints) {
          usersPoints[rawProjects[i].username] = {
            projectPoints:
              rawProjects[i].projectPoints +
              usersPoints[rawProjects[i].username].projectPoints,
            karmaPoints:
              rawProjects[i].karmaPoints +
              usersPoints[rawProjects[i].username].karmaPoints,
            communicationPoints:
              rawProjects[i].communicationPoints +
              usersPoints[rawProjects[i].username].communicationPoints,
            totalPoints:
              rawProjects[i].totalPoints +
              usersPoints[rawProjects[i].username].totalPoints,
          };
        } else {
          usersPoints[rawProjects[i].username] = {
  ...
  render() {
    return (
      <div className="LeaderboardComp">
        <UsersPointsTable tablePoints={this.state.tablePoints} />
      </div>
    );
  }
}
