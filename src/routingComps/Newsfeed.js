...
export default class newsfeeds extends Component {
  ...
  componentDidMount() {
    ...
    // Ensuring that the value of the username is fetched correctly
    setTimeout(
      () =>
        // Getting the posted projects.
        axios
          .get("http://" + ip + ":5000/adminProjects/", {
          ...
          .then((response) => {
            ...
            response.data.reverse().map((project, index) => {
              // Finding the projects that the current user is enrolled in
  ...
  // Modifying the state of a specific button to disabled.
  setDisabledButton = (i) => {
  ...
  // Updating the DB with the new enrolled user.
  updateDB = (e, i) => {
    let ip = window.location.hostname;
    ...
    axios.post(
      "http://" + ip + ":5000/adminProjects/adduser/" + projectId,
      newUser
    );
  };
  ...
  displayCreateButton = (index, length) => {
    if (index == length - 1 && this.state.isAdmin == true) {
      return (
        <Button type="button" href="/create-project">
          Create Project
        </Button>
      );
    }
