...
export default class MyProjects extends Component {
	...
	displaySubmitButton = (id) => {
		if (this.state.isAdmin == false) {
			return <Button onClick={e => {this.submitFiles(e)}} id={id}>Submit</Button>
		}
	}
	...
	onSubmit = (e) => {
		...
		axios.post('http://localhost:5000/projects/upload-file', uploadedFile, {withCredentials: true},{
		    headers: {
		...
		axios.post('http://localhost:5000/projects/submit-files/' + this.state.projectID, newAttachment)
		this.setState({
			isClicked: !this.state.isClicked
		})
	}
	  				...
	  				{this.state.projects.reverse().map((project, index) => {
	  					if(project.username == username){
	  						...
	  						{this.createProjectComponent(project.projectName, project.username, 
	  											         project.projectPoints, project.status, project._id)}
	  					</div>		
	  					}			 	
					})}
	         	</div>
